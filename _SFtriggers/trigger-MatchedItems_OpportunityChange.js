trigger MatchedItems_OpportunityChange on Opportunity (after insert, after update) {
    ...
    for (Opportunity o : Trigger.new) {
        ...
        if( o.Vessels__c != null ) {
            if ( where_sql != '' ) where_sql += ' AND ';  
            where_sql += ' Id = \'' + o.Vessels__c + '\'';
        }      
        system.debug ('-----Vessels__c');

        if( o.Builders__c != null ) {
            if ( where_sql != '' ) where_sql += ' AND ';
            where_sql += ' Builders__c = \'' + o.Builders__c + '\'';
        }
        ...
        if( o.Location_Regions_Name__c != null ) {
            if ( where_sql != '' ) where_sql += ' AND ';
            where_sql += ' Regions__c LIKE \'' + o.Location_Regions_Name__c + '\'';
        }
        system.debug ('-----Regions__c');

        system.debug ( 'WHERE => ' + select_sql + where_sql );

        if ( where_sql != '' ) {
            List <Product2> mProducts = Database.query( select_sql + where_sql + ' LIMIT 50' );
