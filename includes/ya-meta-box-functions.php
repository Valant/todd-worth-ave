 ...
 */
function ya_admin_field( $options ) {
	$type = isset($options['type'])  ? $options['type'] : 'text';
	$func = function_exists('ya_wp_'.$type.'_input') ? 'ya_wp_'.$type.'_input' : 'ya_wp_'.$type;
	if( function_exists($func) ){
		$func($options);
	}else{
		ya_wp_text_input($options);
	}
}
/**
 ...
 * @param array $field
 */
function ya_wp_text_input( $field ) {
 ...
 * @param array $field
 */
function ya_wp_hidden_input( $field ) {
	global $thepostid, $post;
 ...
 * @param array $field
 */
function ya_wp_textarea_input( $field ) {
	global $thepostid, $post;
...
function ya_wp_units( $field ) {
	...
	echo '<p class="form-field form-field-unit ' . esc_attr( $field['id'] ) . '_field ' . esc_attr( $field['wrapper_class'] ) . '"><label for="' . esc_attr( $field['id'] ) . '">' . wp_kses_post( $field['label'] ) . '</label><input type="text" class="unit-value ' . esc_attr( $field['class'] ) . '" style=" width: 25%; ' . esc_attr( $field['style'] ) . '" name="' . esc_attr( $field['name'] ) . '" id="' . esc_attr( $field['id'] ) . '" value="' . esc_attr( $field['value'] ) . '" placeholder="' . esc_attr( $field['placeholder'] ) . '" ' . implode( ' ', $custom_attributes ) . ' /><select id="' . esc_attr( $field['id'] ) . '_unit" name="' . esc_attr( $field['name'] ) . '_unit" class="unit-select ' . esc_attr( $field['class'] ) . '" style="max-width: 100px; width: 25%; ">';
